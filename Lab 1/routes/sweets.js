...
router
    ...
    .get(async (req, res) => {
        try {
            ...
            res.status(500).json({ error: error }); //TODO: 404 when out of sweets vs other error?
        }
    })
    .post(async (req, res) => {
        try {
...
router.route("/?page=:page").get(async (req, res) => {
    try {
        ...
        const listOfSweets = await sweets.getFiftySweets(req.params.page);
        res.json(listOfSweets);
    } catch (error) {
        res.status(404).json({ error: error }); //TODO: 404 when out of sweets vs other error?
    }
});
...
router
    ...
    .patch(async (req, res) => {
        //TODO: how to deal with multiple params?
    });
